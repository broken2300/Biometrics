%function [template, mask] = probe1()
% !!!!!!!!
% need modified
% !!!!!!!!
maindir = 'C:\Users\gban\Desktop\iriscode\LG4000-2010-04-27_29\2010-04-27_29';    % dir for probe1 -  LG4000 as probes
subdir = dir( maindir);
    ...
    for j = 1 : length(subdirG)
        ...
        if subdir( i ).name == subdirG( j ).name
            ...
            [newtemplate, newmask] = createiristemplate(imagepath);
                % get gallary image
                imagepathG = fullfile( maindirG, subdirG( j ).name, imagesG( length( imagesG ) ).name);
                %
                %  caculate
                %
